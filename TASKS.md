...
## AE · Z 回测数据可行性普查（S4，2026-07-17）

- [x] **AE.1 Z=0.84 真实数据回测可行性普查** ✅（本次提交，记录见
  `outputs/S4-Z-BACKTEST-FEASIBILITY.md`）
  `Z=0.84`（`mastery_gate.py` 置信下界 z 分数）目前只在合成数据验证过判别力
  （`scripts/moat_eval/`，AUC 0.677≥0.65 合成门槛；README 已知局限"合成 ≠ 真实"）。
  S4 先做可行性普查：数够不够跑真实回测，不做真跑。新增可重跑脚本
  `scripts/z_backtest_feasibility.py`（合格判据沿用 `mastery_gate.N_MIN`——只有已过
  N_MIN 的 (student,KC) 对，Z 置信下界才会被门控实际用到，用同一口径统计"够不够格"
  最贴合真实使用面，单源、不新引入判据）。
  **实测**：生产库非删除用户仅 13 个；`kc_mastery` 全表 4 行（4 个 (student,KC) 对），
  **合格对（n_attempts≥2）= 0**，远低于 200 门槛（199 个此前已清理的纯种子测试用户
  不会虚增此计数，见 `outputs/SEED-PURGE-RECORD.md`）。
  **决策（按 S4 拍定规则执行）**：合格对 0 < 200 → 不跑真实回测，只记录当前值；推迟
  到真实数据量上来（用户外部跟踪为 W4，不在本仓库另起条目跟踪）；**`Z=0.84` 本身不改、
  不放宽**——已核对 `mastery_gate.py` 当前值仍为 0.84，本轮**零核心代码改动**，只新增
  一个只读普查脚本 + 记录文档。ruff+mypy 过。

## AF · W2 欠账清偿（W2C，2026-07-17，3O v3.0 + FC-1..FC-7）

前置：W2 已关闭（W2-CLOSE-001）；S1-S4 已完成。顺序：C4-0 查证 → C3 → C4 → C1 → C2 → C5。
地基声明（每项都成立）：Mneme 至今零真实学习者，W2C 不得以"为将来用户准备"扩大范围，
五项做完即停。

- [x] **C4-0 Stratum 可达性查证** ✅（本次；前置查证，无代码改动）
  Stratum 确实在跑（`stratum-api`/`stratum-web`/`stratum-sl` 容器健康，
  `/health`→`healthy`，真实 OpenAPI 含 `/api/search`）。但 `mneme-api-1` 在
  `mneme_default`+`helios-net`，`stratum-api` 在独立的 `stratum-net`——两者当前**无网络
  路径**；且 Stratum 鉴权只有用户登录（register/login/refresh），无服务账号/API-key 机制
  可供机器间调用。按 §9 拍定规则：**C4 整项推后，其余照做**——不自行改 Stratum 网络配置、
  不自行搭建。

- [x] **C2 quiz_generator（组卷，复用 AA 段出题/判分）** ✅（本次提交）
  定调：不重造出题/判分，只解决"考哪些 KC、什么顺序"。
  `packages/mneme-core/mneme_core/oprim/quiz_selection.py`：新 oprim `shape_by_difficulty`
  （ascending/diagnostic/mixed 三种难度曲线；无 IRT 题库参数，退化为 KC 级难度，对齐
  `KnowledgeUnit.difficulty`——不假装有 IRT）。`oskill/quiz_generator.py`：组合
  `mastery_gate.is_mastered`（按 p_learned 排除已过门 KC）+ `shape_by_difficulty`，
  纯函数、去重、候选不足不补空。FC-6：带 Mneme 题库/掌握度假设 → mneme-core 私有。
  `KnowledgePoint` 加 `difficulty: float = 0.5` 字段（非破坏性，默认值）；
  `progress_assembler.build_learning_progress` 顺带查出 `KnowledgeUnit.difficulty` 填入
  （同一条既有查询多选一列，不新开查询路径）。
  服务层 `tool_generate_quiz` + `POST /mcp/GenerateQuiz`：候选池不传则用 GetPath 默认路径；
  选中的 KC 序列仍逐个经**既有** RequestQuestion（AA.7/AA.9/AA.10 过滤）出题、
  SubmitAnswer+guard 判分回流 `process_interaction`——**不新开第二条出题/判分路径**，
  S1 的 92.4% 判分 CI 门约束不变（组卷只选 KC，不碰判分逻辑）。
  15 新测试（mneme-core 11 个：难度曲线×3+去重+边界；根仓 4 个：对真实 g10-a 路径断言
  候选子集/难度升序/新学生零掌握度不误排除）。mneme-core 套件 96 过（85 既有+11 新）；
  根仓 ruff+mypy 过，`pytest` 619 过/4 败（与 S1 记录的既有失败同源，无关）。
  ⚠️ `quiz_mimic`（仿真题风格克隆）按 spec 允许推迟到 W4，本次不做。

- [x] **C3 persona（教学人格模板）** ✅（本次提交）
  DeepTutor 精读发现：persona 是纯文本块（Markdown+YAML frontmatter），拼进 system
  prompt 的独立 `persona_style` 块，对工具/权限/判分**零耦合**（"no grant mechanism"）
  ——与 Mneme 红线完全吻合，照此模式设计；但**不照抄 DeepTutor 的用户自建两层模型**
  ——"教学人格模板"是固定预设集，人工把关儿童适龄性，不开放自由创作。
  **FC-6 判定（书面记录）**：模板文案带 Mneme 教学假设（面向中国中小学生、儿童适龄
  措辞）→ 私有；加载/渲染机制形状虽通用，本轮不迁移进 platform/3O——对照 D3 先例，
  避免不成熟共享包变更，留待多项目复用需求明确后再评估（记录见
  `services/persona_store.py` 顶部文档）。
  新 schema `persona.templates`（migration `f7a8b9c0d1e2`）：slug/name/description/body，
  **不含任何判分/门控字段**；种了 3 个人工编写的儿童适龄教学人格（鼓励型伙伴默认/
  干脆型教练/好奇探索者）。`services/persona_store.py`（raw SQL，对照 gate_store.py
  风格）：`list_personas`（不含 body，供选择器）/`get_persona`（含 body）/
  `render_for_prompt`（纯函数拼 system prompt 块）。MCP 工具 `ListPersonas`/`GetPersona`
  + 对应 HTTP 端点（登录闸门，非学生数据无需越权校验，对照 GetKCInfo 惯例）；
  `GetPersona` 查无该 slug 时回落默认人格，不报错。
  **红线测试**（C3-2 验收）：`tests/test_persona_no_gating_coupling.py` 用 AST 解析
  静态断言 `persona_store.py` 不 import 任何门控/判分模块；另断言
  `is_mastered`/`next_objective`/`process_interaction` 签名里没有 persona 参数——
  结构性不可能耦合，非"目前没有"。10 新测试（含 C3-1 验收：切换 persona 后
  `prompt_block` 内容/名称确实不同）。ruff+mypy 过；`pytest` 629 过/4 败（与既有失败
  同源，无关）。
  ⚠️ C3-1 的完整 e2e（真实对话中切换 persona 观察语气变化）留待 C1（chat）建成后
  验证——本次只验证到"数据层：switch persona → prompt_block 确实不同"，符合建设顺序
  预期（C3 本就排在 C1 之前，为 chat 做注入项准备）。

- [x] **C4 Stratum 检索接入** ✅（本次提交，用户拍板重启网络/账号后解除 C4-0 阻塞）
  C4-0 记录的两个阻塞点本次逐一解决：①`mneme-api-1` `docker network connect
  stratum-net`（Stratum 只在宿主机 127.0.0.1 暴露端口，`host.docker.internal`
  够不到，故走 docker network；不改 Stratum 自己的 compose 配置）——同时把
  `stratum-net` 声明进 `docker-compose.yml` 的外部网络+`api` 服务网络列表，让这次
  手动 connect 对未来的容器重建持久化，不再是"记得手动补一次"的隐患；②Stratum 无
  服务账号机制，注册专用 `mneme-rag-service@sxueji.com` service 账号（密码走
  `.env` 的 `STRATUM_SERVICE_EMAIL`/`STRATUM_SERVICE_PASSWORD`，未硬编码）。

  **FC-6 分类筛判定（书面记录）**：任务原文写"obase.rag"，但 `obase` 实测是纯 pip
  装的跨项目共享包（`obase.db.__file__` 落在 site-packages，本仓库无本地 `obase/`
  目录）——改共享包需跨项目发布协调；客户端又绑定 Stratum 具体契约（login/search
  端点形状、`mode=strict|augmented` 等 Stratum 特有概念，非通用抽象）。故不进共享
  `obase`，落 `services/rag_client.py`（Mneme 本地）。

  `services/rag_client.py`：懒登录（token 15min 过期，缓存+到期重登）+ 检索，
  fail-safe（无凭据/网络失败/非 2xx 一律返回空列表，不阻断对话——RAG 缺失不该
  打断呈现）。MCP 工具 `SearchKnowledgeBase` + HTTP 端点（不碰 db，只要求登录，
  同 `ListPersonas` 惯例）。`tutor_loop.build_tools()` 10→11 callable。

  **红线测试**（对照 C3 persona/C5 memory 同一模式）：AST 静态断言
  `rag_client.py` 不 import 任何门控/判分模块；断言门控函数签名无 RAG 相关参数；
  直接证明 `is_mastered` 判定与 RAG 检索结果之间没有数据通路（C4 验收原文"断言
  RAG 结果不影响 is_mastered 判定"）。

  **真实验证**：完整走通 注册→登录→`/api/search`→通过 Mneme `/mcp/
  SearchKnowledgeBase` 代理 的全链路（`packages/mneme-agent/tests/
  test_tutor_loop_rag_tool.py` 真 HTTP，非 mock）。⚠️ 服务账号语料库当前为空
  （零已上传内容），故检索返回空列表——通路验证通过，但"chat 能引用检索到的
  教材内容"要真正生效，还需把 Mneme 教材内容上传/摄入进这个 Stratum 账号，是
  独立的内容填充工作，不在 C4（HTTP 客户端接入）范围内，留后续 task。

  9 新测试（rag_client 5 个假 HTTP + 红线 3 个 + MCP 工具 2 个 + tutor_loop 挂载
  2 个真 HTTP）。ruff+mypy 过。回归：根仓 660 过/4 败（既有失败同源）；
  mneme-core 103 过；mneme-agent 12 过/0 败。

  ⚠️ 两次容器重建（拾取新路由/新环境变量）都经用户确认后执行，第二次重建意外
  丢了手动 `docker network connect`（compose 只管自己声明的网络）——已发现并
  补上 `docker-compose.yml` 的声明式网络，此后重建应自动带上。

- [x] **C1 Chat 工作区：intent_router + 复用 tutor_loop** ✅（本次提交）
  DeepTutor 精读发现两处偏差（不可照抄）：①它**没有** "intent_router" 概念——不做
  自由文本意图分类，模式选择在上游由调用方显式指定 capability_name；②会话持久化是
  本地文件 JSON（agent 进程内 IO），与 Mneme "agent 进程零 DB/零本地持久化" 硬约束
  不兼容。intent_router 的"自由问答/转 Mastery Path"两模式判定按 Mneme 自己设计
  （DeepTutor 无对应机制可抄）；LLM 抽象直接复用 `mneme-agent` 已有 `qwen_llm.py`
  （未引入 DeepTutor 的 client 抽象）；prompt_blocks.py 的具名分块组装模式（含
  persona_style 槽位）照搬到 chat_loop 的 system prompt 拼装。

  **intent_router**（`packages/mneme-core/mneme_core/oprim/intent_router.py`）：
  单 LLM 调用 = oprim（不是 oskill，复杂≠层级）。`classify_chat_intent(message, *,
  llm)` 注入 LLM（对照 qualitative_verifier 同一惯例，mneme-core 零依赖不自己发
  网络请求）；fail-safe：解析失败/字段非法一律回落 free_qa。FC-6：带 Mneme 模式
  假设 → 私有，不进 platform/3O。7 新单测（假 LLM 注入，覆盖 markdown 围栏/非法
  JSON/异常/字段缺失场景）。

  **chat_loop**（`packages/mneme-agent/mneme_agent/assembly/chat_loop.py`，新建，
  平行于 tutor_loop.py）：`run_chat_turn` 先分流意图——practice **完全不进循环**，
  只返回 `{action:"goto_mastery_path", kc_hint}` 交前端导航 /studio/learn 驱动既有
  NextObjective/RequestQuestion 流程（不重造）；free_qa **复用已装配的
  `tutor_loop.build_tutor_loop`**（禁另起循环，FC-4）。多轮对话：`AgenticLoop.session()`
  本身不持久化跨调用历史，调用方把历史拼进单次 task 文本（真正的历史持久化留 C5
  MCP wiring）。工具面 = tutor_loop 既有 8 callable（无 RequestQuestion——bank 的
  expected 天然不进这条路径的上下文，红线自动成立）+ persona（C3，作为 system_prompt
  注入）；rag（C4）因 C4-0 不可达，暂不注入。3 新单测（practice 分支验证 loop_caller
  从未被调用、free_qa 验证 persona 正确拼进 system、多轮历史正确拼装）。

  **服务层**（`services/chat_router.py`，新建）：`POST /v1/chat/turn`。
  `tool_chat_turn` 持全部编排（取 GetPath 默认 KC 池 / 复用 `tool_get_persona`
  单源取 persona / 组装真 Qwen caller），对照 mcp_router.py 的 tool_*/mcp_* 分层
  惯例可脱 HTTP 直测。main.py 挂载走 mcp_router 同款"未装优雅降级"try/except。
  5 新测试：practice 分支不碰 loop、persona 切换 system prompt 确实不同、未知
  slug 回落默认、**expected 断言扩展到 chat**（`"expected" not in
  json.dumps(result)`，对照 `test_mcp_http_acceptance.py:174` 的 W3 断言）。

  **意外但重要的发现+修复**：验证过程中发现 `packages/mneme-agent` 自己的测试套件
  （不在 `scripts/check.sh` 默认跑的范围，故此前无人发现）有 4 个测试挂——根因是
  AA.1 给全部 `/mcp/*` 加了 JWT 强制要求，但 `tutor_loop.py` 的 `_mcp()` HTTP 客户端
  从那以后就没跟着加 `Authorization` 头，导致 agent 侧发起的每个工具调用在真实环境
  100% 401。**这不是本次引入的 bug，只是被 C1 复用 tutor_loop 时验证到**；用户拍板
  "现在就修"。修法：`_post_sync`/`_mcp`/`build_tools`/`build_tutor_loop` 全加
  `auth_token` 透传参数（`build_tools` 内收敛成一个 `_call()` 闭包，8 处调用点只改
  一处）；`chat_router.py` 用 `Depends(oauth2_scheme)` 拿学生这次请求自带的 token
  原样转发（不单独铸造凭据）。同步修复受影响的既有测试（`test_mcp_http_acceptance.py`
  3 个、`test_tutor_loop_e2e.py` 1 个、连带 `test_tutor_loop_fc5.py` 的 subprocess
  runner 加 4th argv 传 token）——harness 侧用 `obase.auth.create_access_token` 现铸
  该学生自己的 token，不是绕过鉴权。复测：4 个坏测试全部转绿。
  ⚠️ 排障过程一个真教训：`cd packages/mneme-agent && pytest` 会把 CWD 移出 `/app`，
  导致 `.env` 找不到、`JWT_SECRET` 静默退化成 dev 默认值，制造出看似的"token 校验
  失败"假象（实际生产无此问题，因为服务进程始终从 `/app` 启动）——本仓库子包测试一律
  应从仓库根跑 `pytest packages/xxx/tests/`，不要 `cd` 进子目录。

  ruff+mypy 过（新文件范围内；仓库既有 6 处 ruff 报错/1 处 mypy vendor/omodul 冲突
  经隔离验证均为他人在制品/预置配置问题，与本次无关，未动）。回归：根仓 `pytest`
  635 过/4 败（与 S1 记录的既有失败同源）；`packages/mneme-core` 103 过；
  `packages/mneme-agent` **8 过/0 败**（原 4 败已修复，非本次新增回归）。
  ⚠️ 范围声明（对照 C2/C3 先例）：本次只交付后端契约（intent_router + chat_loop +
  `/v1/chat/turn`），`apps/mneme-studio/app/chat` 前端接线（真实点击 /studio/chat
  驱动对话、practice 分支真跳转 /studio/learn）留后续 task，非本次范围。

- [x] **C5 三层 Memory follow-ups：MCP wiring + LLM merge + TTL 清理** ✅（本次提交）
  DeepTutor 精读发现的偏差（不可照抄）：`consolidator.merge` 模式**不调 LLM**——纯
  机械式脚注去重整理（跟 S3 已建的 mneme `merge`——机械累积 items——性质相同，不是
  语义合并）。DeepTutor 真正做"LLM 语义合并"的是 `dedup`（同文档内判重复/相似条目、
  保质量更高的一条+refs 并集）和 `update`（增量抽取事实、只增不改）。故 Mneme 自己
  `merge` 模式的 LLM 调用按 DeepTutor `dedup` 的 prompt 策略设计（合并对判断+保留
  高质量表述），而非照搬一个叫"merge"的模块。DeepTutor 三层是
  L1(原始流水)/L2(单来源摘要)/L3(跨来源综合)，没有 working memory 概念，与 Mneme
  的 working/episodic/semantic 不是 1:1 映射——只在 episodic≈L1、semantic≈L2+L3
  的粗粒度上对应。

  **mneme-agent MCP wiring**：四模式（audit/dedup/merge/update）此前只管维护*既有*
  条目，没有"新增"入口——补 `services/memory.append_episode`（写一条 episodic，只增
  不改）+ `recall`（读 semantic_memory，呈现层只读召回）。新 MCP 工具
  `RecallMemory`/`RememberEpisode` + HTTP 端点（`RecallMemory` 走
  `_ensure_student_access`、`RememberEpisode` 走 `_ensure_student_self`，对照既有
  端点鉴权惯例）。`tutor_loop.build_tools()` 8→10 callable（新增两个纳入同一
  `_call()` 闭包，自动带 AA.1 要求的 auth_token）。**验证过程意外触发一次真实部署
  动作**：新路由要生效需重启 `mneme-api-1`（无 `--reload`，代码在盘但进程未加载）；
  这会瞬时中断 api.sxueji.com，用户确认后执行，重启后 HTTP 往返测试转绿。

  **LLM 语义合并**：`merge()` 新增可选 `llm` 参数（注入式，对照 mneme-core
  `qualitative_verifier`/C1 `intent_router` 同一惯例——不在 services 层直接耦合
  具体 provider）；有新 episodic 条目且传了 llm 时，调用生成/更新
  `content.summary`（合并新旧信息、去重复、保留更准确说法，对照 DeepTutor `dedup`
  的判断策略）；LLM 失败不阻断机械合并（fail-safe）；`llm=None` 时行为与 S3 完全
  一致（无 summary 字段，向后兼容，S3 既有 7 测试原样通过）。

  **working-memory TTL 清理**：`services/memory.cleanup_expired_working_memory`
  （删 `expires_at < now()` 的行）+ 新 `tasks/memory_tasks.py`（Celery 任务，同
  `purge_tasks.py` 惯例：任务自建独立 engine）+ `celery_app.py` 注册每日 04:00
  `beat_schedule`。

  **红线测试**（对照 C3 persona 同一模式）：`tests/test_memory_no_gating_coupling.py`
  用 AST 解析静态断言 `services/memory.py` 不 import 任何门控/判分模块；另断言
  `is_mastered`/`next_objective`/`process_interaction` 签名无 memory 相关参数、
  `recall` 签名无写入类参数——结构性不可能耦合。FC-2：本次未新增表（复用 S3 的
  `agent.*` schema），purge 守卫 `test_every_student_table_is_in_purge_list` 复测
  仍绿。

  ⚠️ 排障中一个真教训（已记入 C1 段，此处复述适用范围）：子包测试一律从仓库根跑
  `pytest packages/xxx/tests/...`，不要 `cd` 进子目录——否则 `.env` 找不到、
  `JWT_SECRET` 静默退化。

  22 新测试（memory 服务层 8 个：append_episode/recall/TTL/LLM merge 4 种场景；
  MCP 工具层 3 个；红线测试 3 个；TTL 任务 1 个；tutor_loop 挂载 2 个；另
  mneme-agent 侧因容器重启新增 e2e 覆盖）。ruff+mypy 过（新文件范围内；既有 6 处
  ruff/1 处 mypy vendor/omodul 冲突经隔离验证与本次无关）。回归：根仓 `pytest` 650
  过/4 败（与 S1 记录的既有失败同源）；`packages/mneme-core` 103 过；
  `packages/mneme-agent` 10 过/0 败。

  **W2C 五项当时全部完成**：C4-0 查证（Stratum 不可达，C4 推后）/ C2 组卷 /
  C3 persona / C1 chat 工作区 / C5 memory follow-ups。C4 本身（Stratum 检索接入）
  当时按拍定规则保持推后——后续用户拍板重启网络/账号，见下方 C4 补记，五项
  最终全部落地。

- [x] **S0-W5 全仓沙箱漏洞封堵 + partner_tasks 崩溃修复**（W5 安全/稳定前置，本次提交）
  第 4 次同类洞：W5 前置 sweep 发现 5 个坐在真实学生输入上（chat/判分/OCR手写）的
  未沙箱化点——`verify_step`/`grade_question`/`compute_feedback`（2 处）/
  `socratic_service`/`paper_grading`，全部改走 `obase.sympy_runtime.evaluate_auto()`
  （新增：合并显式变量 + 自动探测单字母自由符号，供不预知变量名的外部输入场景用）。
  self-check 检测方法本身是瞎的（只 grep `sympify(`，`parse_expr()` 完全看不见）——
  按用户要求把方法本身升级为 AST 解析（新 `vendor/obase/sandbox_ast_audit.py`：追踪
  import 别名、解析真实调用目标，拒绝清单而非允许清单）。全仓跑一次，立刻多抓到第 6
  个漏洞——`services/math_grade.py`（`SubmitAnswer` 判分主链路，全仓真实调用量最大
  的外部输入入口之一），此前 5 个已知发现全部来自字符串 grep，这个是纯字符串匹配
  漏掉、AST 扫描才揪出来的，直接验证了这次架构升级的必要性。`obase/sandbox_selfcheck.py`
  的 `_check_zero_bypass` 收窄为只做"内核清单漂移 + 纯数值内核 run_isolated"（AST
  拒绝清单结构上抓不到这类风险，只能专属检查保留）；原 STRING_EVAL_KERNELS/
  AST_VALIDATED_ENTRY_POINTS/VISUALIZATION_KERNELS 允许清单删除，改接
  `_check_repo_wide_ast_audit()`。新 `tests/test_sandbox_ast_audit.py`（11 测试：
  证明抓到 `parse_expr()`、import 别名、module 别名、危险内置函数，且不误报安全用法/
  沙箱实现文件自身/已评审例外/测试目录；含端到端"当前仓库零发现"回归）。

  迁移过程中順带修了两个真 bug（不是预先计划的）：① `SymPyRuntime.evaluate()` 对裸
  数字字面量（如 `"3"`）走原生 Python `eval()` 返回 `int` 而非 `sympy.Integer`，
  破坏下游 `.is_number` 类判断——在 `evaluate()` 源头做 `sp.sympify()` 兜底修复；
  ② 从 `parse_expr(..., implicit_multiplication_application)` 切到纯 `ast.parse`
  丢失隐式乘法支持——加窄正则 `_IMPLICIT_MULT_RE` 补上，但第一版只处理"数字紧跟
  字母"（"2x"），全仓回归时 `test_math_grade.py::test_pi_and_sqrt` 抓到"数字+空格+
  字母"（"2 pi"，LaTeX `\pi` 去反斜杠后的常见形式）被漏解析——三个文件
  （math_grade.py/socratic_service.py/paper_grading.py）统一改为
  `r"(?<=[0-9])\s*(?=[A-Za-z])"` 后复测全绿。

  `tasks/partner_tasks.py`：不是死代码，Celery beat 每天 17:30 真跑，每次崩在
  `User.last_login`/`.username`（模型只有 `.name`，且 `last_login` 不是重命名
  问题——全仓没有任何登录时间戳追踪机制，从未实现）。改为直接用 `.created_at`
  兜底"距注册天数"（非真正"距上次登录"，标注为已知局限，真正的登录时间戳需要
  新增列+migration+登录路径写入，超出本次崩溃修复范围）+ `.username`→`.name`。
  新 `tests/test_partner_tasks.py`（4 测试，真实 DB 行，非 mock）：核心回归"不再
  AttributeError"+ 两条通知分支内容正确 + 负向对照（不该收到的学生不收到）。

  ruff+mypy 全绿（触及文件范围内；顺带清理本轮改动自己产生的死 import，未碰无关
  文件的既有 4 处 ruff 违规）。全量回归：根仓 `pytest` 794 过/4 败/3 跳过——4 败
  与 S1 记录同源（`test_daily_plan.py`×3 + `test_dod_e2e.py`，daily_plan/
  qualitative_verify 相关，跟本次沙箱/partner_tasks 改动无关，未碰这些文件）。

  ⚠️ 未做（用户明确拍板留给正式 W5 spec）：Partners 渠道适配器/事件总线/多用户
  隔离-授权-审计任何实现——本次只做安全前置封堵，未进 Partners 正式建设。

  ⚠️ 待办：改动已落盘到 `.:/app` 挂载卷，但 `api`/`worker`/`beat` 进程未重启，
  运行中进程尚未加载新代码（含 sandbox_selfcheck 的新自检逻辑）——重启会瞬时中断
  `api.sxueji.com`，需用户确认后执行。

  **[后续更新] 已重启生效**：用户确认后 `docker compose restart api worker beat`，
  `api` 启动日志确认新自检通过（"S0 沙箱加固自检通过：vendor/ 生效，全仓 AST 扫描
  零绕过"），`/health` 200，三容器 healthy。

- [x] **W5 Part A：Partners 渠道+心跳+数据隔离**（MNEME-W5-IMPL-SPEC-001，本次提交）
  用户正式下发 `MNEME-W5-IMPL-SPEC-001`（16 渠道/WeCom+Feishu 首发/多用户/CLI），
  先做 Part A。开工前两轮研究（Explore 查本仓既有脚手架 + general-purpose 抓
  DeepTutor 真实源码 partners/multi_user/cli）揭出 3 个 spec 未钉死的真实缺口，
  逐一问用户拍板（均采纳推荐项）：
  ① oservi 目前只是 docker-compose.override.yml 里一条本机 dev 挂载进 `api`
  （注释自写"非正式生产依赖"），`worker`/`beat` 完全没有——A3 心跳改用已跑通的
  Celery beat 触发（`crontab(minute="*/30")`），不把 oservi 依赖扩到 worker/beat
  镜像；throttle/dedup 逻辑照 AlerterEngine 语义直接实现（查 `agent.partner_push_log`），
  不引 oservi import。
  ② push-only v1 的推送文案生成用单次 LLM oprim（`oprim/generate_partner_push_text.py`，
  无 llm_caller 时回落确定性模板），不起 `agentic_loop`/`subagent_runner`——那条
  留到双向对话真正落地时再接。
  ③ Part B「admin 授权」需要的 admin 身份不建表、不碰 `User`/`UserRole`（"原
  mneme 账号体系零改动"红线），留到 Part B 用 `ADMIN_USER_IDS` 环境变量白名单。

  **A1 渠道注册表**：`services/partner_channels.py`，16 渠道（盘点纠正，非 18）
  全登记，仅 `wecom`/`feishu` 激活；`services/wecom/`、`services/feishu/`
  各一套 `base/mock_provider/webhook_provider/factory`（照搬既有
  `services/email/`、`services/sms/` 四件套惯例，env 驱动，默认 mock）；未激活
  渠道调用显式 `NotImplementedError`（不会假装发送成功）。个人微信不在册。

  **A2 事件总线**：`vendor/obase/partner_bus.py`，纯 `asyncio.Queue` 对
  （inbound/outbound），无外部 broker；每 Partner 一实例，非全局单例。W5 v1
  push-only 只用 outbound 侧，inbound 为未来双向入站预留形状。

  **A3 心跳**：`oprim/check_partner_review_due.py`（evaluator，真实 FSRS 到期
  信号——阈值同 `tasks/partner_tasks.py` 既有逻辑，只读不写）+
  `oprim/generate_partner_push_text.py`（单 LLM oprim + 确定性回落）→
  `oskill/partner_dispatch.py`（组合两个 oprim + 按天去重过滤，查
  `agent.partner_push_log`）→ `tasks/partner_heartbeat.py`（Celery task，经
  `PartnerMessageBus` 发布/消费 outbound 消息后调 `send_via_channel` 真实发送 +
  落推送流水）。已注册 `celery_app.py` beat_schedule（每 30 分钟）。

  **A4 数据隔离**：新 migration `c3d4e5f6a7b8`（同一 Postgres 实例既有 `agent`
  schema，无 FK）加两表：`agent.partner_channel_bindings`（学生-渠道-webhook
  绑定）、`agent.partner_push_log`（推送流水，供去重）；同 PR 已把两表加入
  `services/purge_service._STUDENT_TABLES`，`test_every_student_table_is_in_purge_list`
  复测绿。新增 `BindPartnerChannel` MCP 工具（`_ensure_student_self`，未激活
  渠道拒绝写入半个不能用的绑定）。

  **A5 验收**：PA-1/PA-3/PA-4（Celery beat 顶替 oservi，见②）/PA-5/PA-6/PA-7
  全绿（19 个新测试：渠道注册表结构、事件总线收发、真实 FSRS 阈值触发+按天去重、
  心跳端到端 mock 发送+流水落库、红线 AST 断言"不 import 门控/判分模块"、外发
  数据最小化断言）。
  - **PA-2（真实 webhook 推送到真实测试群）未做**——需要用户在 Wiki/环境变量
    配一条真实 WeCom 或 Feishu 群 webhook URL 才能跑，等用户提供后再验。
  - **PA-8（FC-5 零 DB，若独立进程用 pg_stat_activity 监控）**：本次设计下
    Partner 心跳是 Celery task（同 `tasks/partner_tasks.py`/`services/memory.py`
    既有先例，在后端信任边界内直接 DB 访问），不是独立进程——PA-8 原文本身带
    "若独立进程"限定词，本设计下不适用；FC-5 的"零 DB"红线约束的是
    `packages/mneme-agent`（外部 agent 进程，经 HTTP 调 `/mcp/*`），本次 v1 未
    涉及该包，红线未被触碰也未被绕过。此解读已在实现中做了说明，未与用户
    确认——下次汇报时应明确指出这是我方解读，非用户已拍板事项。

  ruff+mypy 全绿；全量回归 `pytest` 813 过/4 败/3 跳过（4 败同 S1 记录同源，
  与本次改动无关）。

  ⚠️ 停在这里等用户：① 是否提供真实 webhook URL 验 PA-2；② PA-8 解读是否认可；
  ③ 是否继续 Part B（多用户）。

  **[后续更新]** 用户确认 PA-8 解读认可，指示继续 Part B。PA-2 仍待用户提供真实
  webhook URL。

- [x] **W5 Part B：多用户（隔离/授权/审计）**（本次提交）
  admin 身份按此前拍板用 `ADMIN_USER_IDS` 环境变量白名单（`obase/admin_identity.py`，
  零 schema 改动，不碰 User/UserRole）。新 migration `d4e5f6a7b8c9`（同一 `agent`
  schema）加两表：`agent.user_grants`（每学生一行，`enabled_tools`/
  `allowed_models` 为 NULL = deny-by-default，非"默认放行"）、`agent.audit_log`
  （append-only，admin 对自己的操作不记——减少噪音，对照 DeepTutor multi_user
  audit.py 同一设计）。`obase/user_grants.py`（get_grant 任何人可查自己、
  set_grant 只有 admin、is_tool_authorized/is_model_authorized 支持 actor=admin
  直接放行）、`obase/audit_log.py`（log_usage/log_admin_action/get_audit_log，
  写失败吞异常不拖垮请求）。

  **真实集成点**：没有空造一个"多用户系统"自证自答——把 Part A 刚建的
  `BindPartnerChannel` 工具接进新授权+审计层：deny-by-default 意味着默认
  没人能绑 Partner 渠道，必须 admin 先 `SetUserGrant` 授权 `BindPartnerChannel`
  才能用；绑定成功落一条 `bind_partner_channel` 审计记录。新增
  `GetUserGrant`/`SetUserGrant`/`GetAuditLog` 三个 MCP 端点（沿用
  `_ensure_student_access`/`_ensure_student_self` 惯例，non-admin 走既有
  IDOR 防护，admin 绕过）。

  **B2/B3 按 spec 原样**：AA.1 既有登录/`/mcp` 鉴权原样复用，未重造；
  classroom/school 层级明确不做（独立 to-B 设计项，本次未碰）。

  **验收**：MB-1（学生 A 查不到学生 B 的授权/审计，403）/MB-2（deny-by-default
  + admin 授权后放行 + 非 admin 不能设置授权）/MB-3（审计记录真实落库）/MB-5
  （IDOR 403）全绿，17 个新测试。MB-4（既有账号体系零改动）以全量回归为证——
  `test_auth.py`/`test_authz.py` 等既有测试原样通过，未碰 User/UserRole。

  ruff+mypy 全绿；全量回归 `pytest` 830 过/4 败/3 跳过（4 败同 S1 记录同源，
  与本次改动无关）。

  ⚠️ 停在这里：Part C（CLI/Agent-native）待续。

  **[后续更新]** 用户指示继续做 Part C。

- [x] **W5 Part C：CLI + Agent-native**（本次提交）
  新 `cli/mneme_cli.py`（stdlib argparse，不引入新依赖如 typer/click——Master
  未列出，需先改 Master 才能加）：登录/whoami/复习队列/出题/提交作答/绑定
  Partner 渠道/admin 授权/审计查询共 8 个子命令，全部经 `MnemeClient` 打
  `/v1/auth/*`、`/mcp/*`——不直连 DB、不 import oprim/oskill/omodul，结构上
  不可能绕过服务层护栏（红线测试：`tests/test_cli_no_guard_bypass.py` AST 断言
  零违规 import + 硬编码路径全部落在 `/v1/auth/*`|`/mcp/*` 下）。token 落本地
  `~/.config/mneme/token`（0600），不重造登录。

  测试两层：① `httpx.MockTransport`（httpx 自带，不引新依赖）验证每个子命令
  真的发对端点/payload；② 两条**真实端到端**测试直接打本机正在跑的 api 服务
  （同容器回环 `http://localhost:8000`）——`whoami` 拿到真实学生身份；用学生 A
  的 token 查学生 B 的复习队列，真实服务器的 `_ensure_student_access` 一样
  403 拦下，证明 CLI 没有绕过任何既有护栏（不是"结构上应该"，是真的拦下来了）。

  `SKILL.md`（命令表 + 典型工作流 + Partners 需 admin 先授权的提醒）+
  `AGENTS.md`（Tools/Capabilities 两层入口、红线清单、3O 分层要点、关键文件
  地图、oservi/自检的环境说明）均已写好，仓库根新增（`apps/mneme-studio/
  AGENTS.md` 是 Next.js 脚手架自动生成的无关文件，未碰）。

  ruff+mypy 全绿（触及文件范围内）；全量回归 `pytest` 839 过/4 败/3 跳过
  （4 败同 S1 记录同源，与本次改动无关）。

- [x] **W5 全量验收 + 关闭**（本次提交）
  Part A（Partners）→ Part B（多用户）→ Part C（CLI/Agent-native）三部分
  均已交付，逐部验收，未出现"未绿就往下一部分走"的情况。全局不变式逐条复核：
  掌握度写入唯一路径+guard（Partner 不自行判定，AST 断言）/ studio-agent-
  partner 零 mneme-DB（Partner 心跳是 Celery task、非独立进程，`packages/
  mneme-agent` 本身未被触碰仍保持零 DB；PA-8 的这一解读已获用户确认）/ 门控
  上游内容下游（未改动）/ 确定性判分路由不变（未改动）/ 原 mneme 零改动
  （User/UserRole 表全程未碰，`test_auth.py`/`test_authz.py` 全绿为证）/ 主库
  元素不可变（未改任何既有 solve_*/oprim 内核签名）/ 不耦合 Stratum（未碰）/
  FC-2（本次新增 6 张 `agent.*` schema 表——`partner_channel_bindings`/
  `partner_push_log`/`user_grants`/`audit_log`——全部同 PR 入
  `services/purge_service._STUDENT_TABLES`，`test_every_student_table_is_in_purge_list`
  全程保持绿）。全部通过。

  全仓 ruff/mypy：改动文件范围内全绿；全仓级 `ruff check .`/`mypy .` 仍各有
  4 处/1 处既有问题，与本次改动无关（`mcp_client.py`/`textbook_qa_service.py`/
  `test_mcp_write_path.py` 的未用 import，`vendor/omodul` 双重模块名冲突——
  后者是全仓级 mypy 的既有已知问题，早前 C5 阶段就记录过"隔离验证与本次
  无关"，非本次引入）。全量回归 `pytest` 839 过/4 败/3 跳过。

  **明确不做（按 spec §6 拍板，均未碰）**：个人微信、16 渠道中另外 14 个的
  真实激活、Partner 双向入站、外部消息 broker、classroom/school 层级、
  Animator/Research/Notebook/Co-Writer、pgvector、挂接精度、Z 回测。

  **仍待用户处理的两件事**（W5 完成不等于这两个消失）：
  - **PA-2**：真实 webhook 推送到真实测试群——需要用户提供一条真实 WeCom 或
    Feishu 群 webhook URL 才能验，目前只验证过 mock provider 路径。
