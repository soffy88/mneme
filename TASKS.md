...
- [ ] **C1 Chat 工作区** 研究已完成，发现与 spec 假设的偏差：DeepTutor **没有**
  "intent_router" 概念——它不做自由文本意图分类，模式选择在上游由调用方显式指定
  capability_name（`LoopCapability` 组合模型：多个能力可同时挂在同一循环上，各自贡献
  工具+system prompt 块+可选 pre-loop）。DeepTutor 的会话持久化是本地文件 JSON（`agent`
  进程内 IO），与 Mneme "agent 进程零 DB/零本地持久化、全经 MCP 工具" 的硬约束不兼容，
  需要把它的 `BaseSessionManager` 接口换个实现（对接 HTTP/MCP 而非文件）。LLM 抽象
  （OpenAI 兼容端点直通）与 `mneme-agent` 已有的 `qwen_llm.py` 高度重合，可直接复用现有
  实现而非引入 DeepTutor 的 client 抽象。prompt_blocks.py 的具名分块组装模式（含
  `persona_style` 槽位）值得照搬。intent_router 的"自由问答/转 Mastery Path"两模式判定
  需要 Mneme 自己设计（DeepTutor 无对应机制可抄）。
