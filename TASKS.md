...
- [~] **U.19 [P2] L4 物理概念优先范式（本轮只做物理）** 🔄 2026-07-04
  ...
  🔜 后续：英语词汇FSRS+分级泛读拆分新 task（需先解决种子数据来源）。
...
- [~] **U.24 [P2] L9 教学机制 feature-flag 化（pedagogy/01-08 已挂，Learner Model 服务化未做）** 🔄 2026-07-04
  ...
  ⏳ **未做**：Learner Model 服务化独立部署——这是把 `services/learner_model.py` 拆成单独
  部署的服务，供各学科端点消费，属于架构级拆分，跟"给机制加开关"是两件不同量级的事，
  本次不做，留作独立任务。mneme 自己的文件走 bind mount，`docker compose restart` 即生效。
