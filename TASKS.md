...
- [ ] **X.1 [P1] coverage 配置 bug：async DB 调用覆盖率被系统性低估**
  ```
  pyproject.toml [tool.coverage.run] 缺 concurrency=["greenlet","thread"]。
  全异步 SQLAlchemy 项目，await db.execute() 切 greenlet，覆盖率工具跟丢。
  实测修复后 main.py 53%→78%/alert_service.py 41%→80%/mission_service.py
  62%→82%/socratic_service.py 69%→84%。check.sh 覆盖率门禁半年来一直没测到
  它自以为测到的东西。一行配置改动，杠杆最高，先做。
  ```

- [ ] **X.2 [P1] 补红线测试：同源自检（lesson_page 图示/答案/末步一致性）**
  ```
  self_check_passed 字段有实现（GET /v1/lesson/{question_id}），但仓库测试
  零覆盖——CLAUDE.md 自己写的红线，"违反即task未完成"，目前没人守。
  ```

- [ ] **X.3 [P1] 补红线测试：沙箱红线（sympy 病态输入超时）**
  ```
  全仓库 grep sympy_runtime/RuntimeConfig/pathological/timeout 零匹配。
  沙箱实现在 pip 装的 obase 包里（独立内核仓库，非本仓库 vendor/ 副本），
  需确认本仓库测试范围内怎么验证/是否只能验证调用方超时处理。
  ```

- [ ] **X.4 [P2] DB 孤儿字段/表清理**
  ```
  - users.province：157用户全默认值，零代码读写，跟 textbook_id 一模一样的
    模式，直接删（migration）
  - exams / daily_reports / learning_patterns 三张表：建表至今从未写入
    （只有 GDPR purge 代码碰过，那部分必须留），评估是否删表或保留待用
  - guardian_consents.ip_address：合规相关，看起来是设计时想采集监护人同意
    IP留痕但从没真正写入——评估是补上还是删掉
  ```

- [ ] **X.5 [P1] 排查 bkt_priors.grade 格式不一致是否为活跃 bug**
  ```
  bkt_priors.grade 100%用"高一/高二/高三"，textbooks/users 等其它表统一用
  G10/G11/G12。先确认有没有代码按年级 join/filter bkt_priors——如果有，会
  静默查不到数据、悄悄退化成默认先验值，是活跃bug不只是脏数据；如果没有
  代码这样查，降级为单纯数据规范化任务。
  ```

- [ ] **X.6 [P3] 12个孤儿 omodul 处置**
  ```
  最有价值：parent_review.py（家长端错题归因历史视图，完整实现但零接线，
  家长现在只能收到告警点不进详情）、cognitive_diagnosis.py（DINA认知诊断+
  能力估计，完整实现零接线）——评估是否接入服务层。
  纯重复/被取代，建议直接删：analyze_paper.py（被 omodul.paper 取代）、
  socratic_tutor_session.py（被 socratic_session_workflow 取代）、
  grade_paper_workflow.py、essay_review_workflow.py（被 oskill.essay_guide
  取代）、learning_progress_report.py（被 learning_metrics_service 取代）、
  knowledge_profiling_workflow.py（peer_percentile 已直接在main.py用）。
  已废弃无消费者：register_ku.py、register_ku_ontology.py、
  verify_knowledge.py、weekly_review_workflow.py、adaptive_quiz_session.py。
  ```

- [ ] **X.7 [P2] 低覆盖率高风险代码段补测试（X.1 修复后重新评估范围）**
  ```
  - main.py _assert_prod_safety（prod 启动安全门，JWT_SECRET/SMS_PROVIDER
    校验）：ASGITransport 不触发 lifespan，需要直接调用测试
  - main.py get_lesson 非缓存路径：历史真实事故("原仅flush无commit"注释)+
    绑同源自检红线的 self_check_passed，目前零覆盖
  - alert_service.py 5个告警阈值分支+落库去重路径：家长告警功能，目前0个
    家长账号在库（低即时影响，但上线前该补）
  - mission_service.py 老用户任务生成路径（116-162行）：只测了冷启动/已存在
    两个分支，日常主路径没测
  - socratic_service.py verify_step 拦截"抓到错误步骤"的成功路径：目前只测
    了无错误的直通路径，拦截本身（含 322-325 顺序颠倒分支）零覆盖
  - storage.py delete_file：零调用方的死代码，且吞掉 S3Error 异常
    （except S3Error: pass），若未来接入合规删除流程会静默隐藏失败，先记录
  ```
