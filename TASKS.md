...
- [ ] **C3 persona（教学人格模板）** 研究已完成（DeepTutor `services/persona`+`skills/`
  精读），设计中。关键发现：persona 是纯文本块（Markdown+YAML frontmatter：
  name/description/body），拼进 system prompt 的独立 `persona_style` 块，对工具/权限/
  判分**零耦合**（"no grant mechanism"）——与 Mneme 的红线（persona 只改"怎么讲"、不改
  "学什么/过不过门"）完全吻合，可直接照此模式设计。
