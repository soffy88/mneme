...
- [~] **V.1 [P1] daily-plan 任务点击死链修复** 🔄 2026-07-09
  ...
  **除数学外其它学科从来没有"跳到具体知识点做题"的真实闭环**——`/subjects/physics/practice`
  ...
  check.sh 全绿。
  ⏳ **未完成**：前端半边（`SubjectHub.tsx`/`home/page.tsx` 里两份几乎重复的 `goTask` 路由逻辑
  统一改用 `subject` 过滤跳转；`error-journal/page.tsx` 读 `useSearchParams` 的 `subject` 传给
  `getErrorJournal`；错题条目点击"重做"按 `item.subject` 分流而非硬编码数学）留待 mneme-web
  会话。
