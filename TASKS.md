...
- [ ] **C5 三层 Memory follow-ups** 研究已完成，发现与 spec 假设的偏差：DeepTutor 的
  `consolidator.merge` 模式**不调 LLM**——纯机械式脚注去重整理（跟 S3 已建的 mneme
  `merge`——机械累积 items——性质相同，不是语义合并）。DeepTutor 真正做"LLM 语义合并"的
  是 `dedup`（同文档内判重复/相似条目、保质量更高的一条+refs 并集）和 `update`
  （从新 episodic 增量抽取事实、只增不改）。Spec 要求"merge 模式接 qwen"，若照抄
  DeepTutor 字面命名会文不对题——需要按 DeepTutor `dedup` 的 prompt 策略（合并对判断+
  保留高质量表述+refs 并集）设计 Mneme 自己 `merge` 模式的 LLM 调用，而不是照搬一个叫
  "merge"的模块。DeepTutor 三层是 L1(原始流水)/L2(单来源摘要)/L3(跨来源综合)，没有
  "working memory" 概念，与 Mneme 的 working/episodic/semantic 不是 1:1 映射——只在
  episodic≈L1、semantic≈L2+L3 的粗粒度上对应。
