...
- [ ] **0.1 [P0]** 3O 骨架与配置：建 `oprim/ oskill/ omodul/ obase/ services/ data/ alembic/ tests/`；迁已实现内核入位；`obase/config.py`(pydantic-settings)、`obase/db.py`(async session)、`pyproject.toml`、`.env.example`。DoD：`pytest tests/test_engine.py` 新结构下通过。
- [ ] **0.2 [P0]** docker-compose（api+postgres16+redis7+minio）。DoD：`docker compose up -d` 全健康。
- [ ] **0.3 [P0]** Alembic async + baseline。DoD：`alembic upgrade head` 成功。
- [ ] **0.4 [P1]** CI 质量门：pytest+ruff+mypy+覆盖率阈值。DoD：一条命令跑完全检查。

## Epic 1 · 持久化（接已有内核）

- [ ] **1.1 [P0]** 全部 SQLAlchemy models（Master §7 全表+枚举）+ autogenerate migration。DoD：建出所有表，字段与 Master 一致。
- [ ] **1.2 [P0]** StateStore 抽象 + PgStore（重构 `cognitive_state.py`，保留 InMemoryStore）。DoD：同序列 InMemory 与 Pg 结果一致。
- [ ] **1.3 [P0]** `omodul` 认知落库：`process_interaction` 落 `kc_mastery` + 追加 `interaction_events`（只增不改），严守更新顺序红线。DoD：两表正确写入。
