...
- [ ] **Y.4-d 真上线时设 MNEME_ENV=prod**：Y.3 已把它接进 compose，但默认dev。
  设prod后 _assert_prod_safety 会强制：JWT非默认(已满足)+SMS=aliyun(待Y.4-c)。
...
- **前端**：✅ 上线就绪。类型/构建全过，无死链，历史误路由已修，冒烟全200，
  prod配置指向真实后端(mneme-api.uex.hk)。人工UX/真后端E2E需人过一遍。
...
- [~] **Z.3 前端登录/注册页 手机号→邮箱** 🔄 mneme-web PR #17 待合并
...
### 云模型（Y.4-a/b）⛔ 阻塞在"没有可用的云密钥"

- [~] **Z.4 文本+视觉改用阿里云（通义千问+通义千问VL）** 🔄 代码就绪，待有效key
  用户决策：文本视觉都用阿里云（中国网信办备案合规）。
  ✅ 已完成（commit d4b3de5）：
    - services/providers/qwenvl_caller.py：Qwen-VL 视觉适配器（DashScope 多模态，
      内核只支持 Anthropic/Gemini 视觉，中国合规视觉自建），3 单测
    - setup.py MNEME_LLM=qwen 分支：文本 qwen-plus + 视觉 qwen-vl-max 设为 default；
      绕过内核 register_default_providers 的坑（QWEN_API_KEY 占位符 truthy 短路
      盖掉真 DASHSCOPE key），直接从 os.environ 读 DASHSCOPE_API_KEY 自建
    - docker-compose api+worker：MNEME_LLM ollama→qwen + DASHSCOPE_API_KEY 透传
    - 装配逻辑已用 fake key 验证：default LLM=QwenCaller、VLM=QwenVLCaller
  🔜 **上线前你需要做**：把一个有效 DASHSCOPE_API_KEY（阿里云百炼 sk-...）放进
  .env，重启后我实测一次苏格拉底(文本)+一次拍卷OCR(视觉)成功。当前环境无有效
  key（DEEPSEEK 401失效、DASHSCOPE占位符），未配 key 时不会真正生效——所以没
  重启进 qwen 态（会回落mock，不比现状好）。给 key 一起做+实测。
