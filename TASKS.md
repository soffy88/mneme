...
- [ ] **Z.4 文本LLM+视觉OCR 改用云模型** ⛔ 需要你提供有效云密钥
  provider 层已内置云支持（register_default_providers 按 key 优先级自动选：
  文本 DeepSeek>Qwen>Anthropic>OpenAI；视觉 Anthropic>Gemini）。唯一挡路的是
  **没有一个能用的云密钥**——实证：现有 DEEPSEEK_API_KEY 调用返回 401
  （失效，非余额问题），Anthropic/Qwen/Gemini/OpenAI 全是占位符 your_key_here。
  切换步骤（拿到有效key后一步到位）：
    1. 你给一个有效云密钥（文本任选DeepSeek/Qwen/Anthropic/OpenAI；视觉OCR
       须Anthropic或Gemini——DeepSeek/Qwen纯文本无视觉，且代码无中国备案视觉
       provider，要中国合规视觉得我另写Qwen-VL适配器）
    2. docker-compose api+worker 去掉 MNEME_LLM=ollama 强制覆盖（改用云）
    3. 填对应 *_API_KEY 环境变量，重启，我实测一次苏格拉底+一次OCR成功
  ⚠️ 不在没有有效key时盲目去掉ollama覆盖——那样会回落到mock（假LLM），比
  现状（ollama 404）没更好。等你给key一起做+实测。
