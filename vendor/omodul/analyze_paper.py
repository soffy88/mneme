...
async def analyze_paper_workflow(
    ...
    *,
    ...
    # 1. OCR
    if on_step:
        on_step({"step": "ocr", "status": "started"})

    # 目前简化：处理第一张图
    if not input_data.image_b64_list:
        return standard_return(
            findings=None, status="failed", error="No images provided"
        )

    ocr_res: PaperOCRResult = await ocr_paper(
        image_b64=input_data.image_b64_list[0], subject=config.subject
    )
    ...
    for q in ocr_res.questions:
        ...
        res = await process_single_question(
            ...
            # T.6：OCR 提取的学生手写步骤（无步骤 → [] → 行为与基线一致）
            student_steps=q.get("student_steps") or [],
        )
        ...
        if res["status"] == "correct":
            ...
            for kc_id in res.get("knowledge_points", []):
                ...
                from oskill.cognitive_state import (
                    cognitive_update,
                ...
                # 见 oskill.cognitive_state，classify 红线公式不变）
                upd_input = CognitiveUpdateInput(
                    state=state,
                    card_dict=card_dict,
                    is_correct=False,
                    step_evidence=res.get("step_evidence"),
                ...
                # 保存
                await store.save(
                    input_data.student_id, kc_id, upd_res.state, upd_res.card_dict
                )
    ...
    stmt = (
        update(Paper)
        .where(Paper.id == input_data.paper_id)
        .values(status=PaperStatus.done, ocr_result=ocr_res.model_dump())
    )
...
__manifest__ = {
    ...
    "elements": [
        {
            "name": "analyze_paper_workflow",
            "layer": "omodul",
            "summary": "试卷分析全流程（OCR+批改+分析+内核更新）",
        },
    ],
}
