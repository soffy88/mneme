...
from obase.notify.telegram import (
    telegram_send as send_message,
)  # 假设使用现有的通知通道
...
async def due_recall_push_workflow(
    config: DueRecallPushConfig, input_data: DueRecallPushInput, *, pool: Any
) -> dict:
    """处理召回推送。"""
    ...
    for item in input_data.due_items:
        ...
        try:
            ...
            push_count += 1
            trail.append(
                {"event": "pushed", "student_id": str(student_id), "kc_id": kc_id}
            )
        except Exception as e:
            errors.append(str(e))
            trail.append(
                {"event": "push_failed", "student_id": str(student_id), "error": str(e)}
            )
