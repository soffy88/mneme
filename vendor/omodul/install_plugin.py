...
from oskill import compose_plugin_manifest
import json
import re
import sys
import os
...
async def install_plugin(
    ...
    input_data: InstallPluginInput,
    output_dir: Path,
    *,
    on_step=None,
) -> dict:
    """解析并安装插件 bundle（写入 install_dir）。

    支柱：decision_trail + fingerprint
    """
    ...
    bundle = input_data.plugin_bundle
    plugin_name = bundle.get("name", "unknown")
    version = bundle.get("version", "0.0.0")
    fingerprint = compute_fingerprint({"plugin_name": plugin_name, "version": version})

    try:
        trail.record(event="install_start", plugin=plugin_name, version=version)
        if on_step:
            on_step({"event": "install_start", "plugin": plugin_name})

        # 解析 manifest（使用 compose_plugin_manifest oskill）
        manifest = compose_plugin_manifest(bundle)
        trail.record(event="manifest_parsed", name=manifest.name)

        # 写入 install_dir
        install_path = Path(input_data.install_dir)
        install_path.mkdir(parents=True, exist_ok=True)
        manifest_path = install_path / f"{plugin_name}.json"
        ...
        trail.record(event="manifest_written", path=str(manifest_path))

        if on_step:
            on_step({"event": "install_done", "plugin": plugin_name})
    ...
    return build_result(
        ...
        install_path=str(install_path / f"{plugin_name}.json") if status == "completed" else None,
    )
