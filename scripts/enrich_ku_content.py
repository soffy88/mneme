...
# ── 连接配置 ──────────────────────────────────────────────────
# 默认走宿主机直连（历史用法）；容器内跑（如 crontab 走 docker compose exec）
# 传 DATABASE_URL_SYNC=postgresql://postgres:postgres@db:5432/mneme 覆盖。
DB_DSN = os.environ.get(
    "DATABASE_URL_SYNC",
    "host=localhost port=5433 dbname=mneme user=postgres password=postgres",
)
...
LLM_API_KEY = (
    os.environ.get("LLM_API_KEY") or os.environ.get("DEEPSEEK_API_KEY", "") or "local"
)
...
def _base_info(ku: dict) -> str:
    return (
        f"知识点：{ku['name']}（{ku['grade']} {ku['subject']}）\n"
...
def run(
    ...
    print(f"\n{'=' * 60}")
    print(
        f"科目: {subject}  |  模式: {'重试失败(_error/_raw)' if retry_failed else '正常(IS NULL)'}"
        f"  |  年级: {grade_label}"
    )
    ...
    def worker(ku: dict):
        ...
        try:
            ...
            if result is None or "_error" in result or "_raw" in result:
                msg = (
                    (result or {}).get("_error", "no result") if result else "no result"
                )
