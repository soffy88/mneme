...
from services.models import (
    AlertLevel, AlertType, DailyMission, InteractionEvent,
    ParentAlert, SocraticSession, KCMastery,
)


async def get_student_alerts(db: AsyncSession, student_id: uuid.UUID, parent_id: uuid.UUID) -> list[dict]:
    """Read unread parent_alerts for a student."""
    rows = (await db.execute(
        select(ParentAlert)
        .where(ParentAlert.student_id == student_id, ParentAlert.parent_id == parent_id)
        .order_by(ParentAlert.created_at.desc()).limit(20)
    )).scalars().all()
    return [
        {"id": str(r.id), "type": r.alert_type.value if r.alert_type else None,
...
async def run_alert_checks(db: AsyncSession, student_id: uuid.UUID, parent_id: uuid.UUID) -> list[dict]:
    ...
    if consecutive_miss >= 3:
        alerts.append({"type": AlertType.task_missing, "level": AlertLevel.important,
                       "content": f"连续 {consecutive_miss} 天未完成任务"})

    # 3. time_drop: interaction count this week vs last week
    this_week = (await db.execute(
        select(func.count()).select_from(InteractionEvent)
    ...
    last_week = (await db.execute(
        select(func.count()).select_from(InteractionEvent)
        .where(InteractionEvent.student_id == student_id,
               InteractionEvent.occurred_at.between(now - timedelta(days=14), now - timedelta(days=7)))
    )).scalar() or 0
    if last_week > 0 and this_week < last_week * 0.5:
        alerts.append({"type": AlertType.time_drop, "level": AlertLevel.attention,
    ...
    if late >= 2:
        alerts.append({"type": AlertType.late_night, "level": AlertLevel.notice,
                       "content": f"近3天有 {late} 次深夜（23点后）答题记录"})

    # 5. score_drop: KCs with consistent negative trend
    mastery_rows = (await db.execute(
        select(KCMastery).where(KCMastery.student_id == student_id, KCMastery.p_mastery <= 0.3)
    )).scalars().all()
    if len(mastery_rows) >= 3:
        alerts.append({"type": AlertType.score_drop, "level": AlertLevel.attention,
                       "content": f"{len(mastery_rows)} 个知识点掌握度低于30%"})

    # Write alerts to DB and return
    written = []
    for a in alerts:
        pa = ParentAlert(
            id=uuid.uuid4(),
        ...
        db.add(pa)
        written.append({"type": a["type"].value, "level": a["level"].value, "content": a["content"]})
    if alerts:
        await db.flush()
    return written
