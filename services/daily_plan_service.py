...
def _is_late_night(
    now: datetime, hour: int = _LATE_NIGHT_HOUR, minute: int = _LATE_NIGHT_MINUTE
) -> bool:
...
async def build_daily_plan(
    ...
    budget_minutes=None（默认）→ 回退到该学生 daily_plan_prefs 里的持久化默认值
                    （同样可能是 None=不限）；传入具体分钟数 → 临时覆盖，按 priority
                    顺序贪心纳入任务，超预算的任务整体砍掉（不拆分单任务），砍掉的
                    任务列在 dropped_tasks 里，不静默丢弃。
    晚间截止（late_night）/薄弱知识点上限/新知识点上限同样读该学生的
    daily_plan_prefs（V.2，GATE 掌握度阈值除外，见 daily_plan_prefs_service.py）。
    """
    if now is None:
        now = datetime.now(timezone.utc)

    # ── V.2 每日计划参数可见+可配置：学生自调的 budget_minutes/late_night_*/
    # weak_max_items/new_max_items（GATE 不在此列，单源常量见 learner_model.py）。
    from services.daily_plan_prefs_service import get_daily_plan_prefs
    ...
    # ── 0.5 会话时间设计（L5，U.20）：晚间截止后不排新任务，睡眠巩固优先。
    # 截止时间可由学生在 daily_plan_prefs 里自调（默认 22:30）。
    late_night = _is_late_night(
        now, prefs["late_night_hour"], prefs["late_night_minute"]
    )
        ...
        tasks.append(
            {
                ...
                "estimated_minutes": min(n, prefs["weak_max_items"])
                * MINUTES_PER_WEAK_KU,
            }
        )
        ...
        tasks.append(
            {
                ...
                "priority": 4,
                "reason": "按课程进度，前置已掌握",
                "estimated_minutes": min(n, prefs["new_max_items"])
                * MINUTES_PER_NEW_KU,
            }
        )
    ...
    # V.2：请求显式传 budget_minutes 时临时覆盖（保留既有能力）；不传则回退到学生
    # 在 daily_plan_prefs 里设置的持久化默认值（同样是 None=不限）。
    if budget_minutes is None:
        budget_minutes = prefs["budget_minutes"]
