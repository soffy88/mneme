...
口径：纯只读重放（不写库）。对每张 (student,kc) 卡片：用默认先验起 KCState，
...
async def reconstruct_eval_logs(db: AsyncSession, student_id=None) -> list[list[tuple]]:
    """每张 (student,kc) 卡片 → [(occurred_at, is_correct, fsrs_rating, item_difficulty), ...]。"""
    stmt = select(
        InteractionEvent.student_id, InteractionEvent.knowledge_point,
        InteractionEvent.occurred_at, InteractionEvent.is_correct,
        InteractionEvent.fsrs_rating, InteractionEvent.item_difficulty,
    )
...
def compute_predictions(seqs) -> tuple[list[float], list[int]]:
    ...
    for seq in seqs:
        ...
        for ts, correct, rating, diff in seq:
            ...
            bkt_update(state=state, is_correct=correct, retrievability=r, difficulty=diff)
            if Rating is not None and rating is not None:
                try:
                    card = fsrs_review(card_dict=card, rating=Rating(int(rating)), now=ts)
                except Exception:
                    pass
...
def predictive_metrics(seqs) -> dict:
    ...
    n = len(preds)
    if n == 0:
        return {"n": 0, "auc": None, "logloss": None, "base_rate": None, "n_cards": len(seqs)}
    pos = sum(actuals)
    logloss = -sum(
    ...
    return {"n": n, "n_cards": len(seqs), "auc": auc,
            "logloss": round(logloss, 6), "base_rate": round(pos / n, 4)}


async def evaluate_model(db: AsyncSession, student_id=None) -> dict:
    """对（全体或某学生的）真实作答评估内核预测力。"""
    seqs = await reconstruct_eval_logs(db, student_id=student_id)
    m = predictive_metrics(seqs)
    if m["auc"] is not None:
        m["verdict"] = ("护城河兑现(≈目标0.77)" if m["auc"] >= 0.75
                        else "优于随机但未达目标" if m["auc"] >= 0.55
                        else "接近随机(模型未体现预测力)")
    return m
