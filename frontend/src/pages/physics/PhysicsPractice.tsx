...
export function PhysicsPractice() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      ...
      <h1 className="text-xl font-bold text-slate-900 mb-2">物理练习</h1>
      {kuId && (
        <p className="text-sm text-slate-500 mb-6">知识点：{kuId.split("-ku-")[1] ?? kuId}</p>
      )}
      ...
      <div className="bg-indigo-50 border border-indigo-200 rounded-xl p-5 mb-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h2 className="font-semibold text-indigo-900 mb-1">受力分析苏格拉底引导</h2>
      ...
      <div className="bg-slate-50 border border-dashed border-slate-300 rounded-xl p-6 text-center">
        ...
        <h2 className="font-semibold text-slate-700 mb-2">物理题库建设中</h2>
        <p className="text-sm text-slate-500 leading-relaxed">
          当前物理题库暂未导入。
          <br />
          已有数学题库 20,000+ 题可参考练习形式。
          <br />
          <span className="text-xs text-slate-400 mt-1 block">物理题库上线后，将支持按知识点自动出题和变式练习。</span>
        </p>
        <div className="mt-4 flex justify-center gap-3">
          <Link
            to="/subjects/physics/lesson"
            className="px-4 py-2 text-sm bg-white border border-slate-300 rounded-lg text-slate-600 hover:border-slate-400"
          >
            返回知识体系
          </Link>
        </div>
