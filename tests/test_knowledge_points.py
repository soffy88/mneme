...
async def seed(db: AsyncSession):
    ...
    db.add(
        KnowledgeCluster(id=c1_id, textbook_id=tb_id, name="第一章", display_order=1)
    )
    db.add(
        KnowledgeCluster(id=c2_id, textbook_id=tb_id, name="第二章", display_order=2)
    )
    await db.flush()
    ...
    db.add(
        KnowledgeUnit(
            ...
            question_types=["选择题"],
            ku_type="concept",
            curriculum_standard="B4.1",
            mastery_levels=[{"level": 1, "label": "认识"}],
        )
    )
    db.add(
        KnowledgeUnit(
            ...
            name="集合运算",
            description="交并补",
            prerequisites=[ku1_id],
            related_kus=[],
            difficulty=0.5,
            exam_frequency="high",
            question_types=["选择题", "解答题"],
            ku_type="method",
            curriculum_standard="B4.2",
            mastery_levels=[],
        )
    )
    db.add(
...
async def test_get_single_ku_not_found(student):
    async with AsyncClient(
        ...
        r = await c.get(
            "/v1/knowledge-points/does-not-exist", headers=_headers(student)
        )
...
@pytest.mark.asyncio
async def test_verified_field_present_and_sorted_first(db, student):
    """列表响应带 verified 字段，且 verified=True 的 KU 排在 unverified 之前。"""
    tb_id = f"test-tb-vf-{uuid.uuid4().hex[:8]}"
    c_id = f"test-c-vf-{uuid.uuid4().hex[:8]}"
    ku_u = f"test-ku-unv-{uuid.uuid4().hex[:8]}"
    ku_v = f"test-ku-ver-{uuid.uuid4().hex[:8]}"
    ...
    await db.flush()
    db.add(KnowledgeCluster(id=c_id, textbook_id=tb_id, name="校验章", display_order=1))
    await db.flush()
    # 故意让 unverified 的 id 排在前（默认 chapter 排序按 ku.id），
    # 用于证明 verified 优先重排生效
    for ku_id, verified in [(ku_u, False), (ku_v, True)]:
        db.add(
            KnowledgeUnit(
                id=ku_id,
                textbook_id=tb_id,
    ...
    try:
        ...
        data = r.json()
        assert len(data) == 2
        assert all("verified" in d for d in data)
        assert data[0]["id"] == ku_v and data[0]["verified"] is True
        assert data[1]["id"] == ku_u and data[1]["verified"] is False
    finally:
        await db.execute(
