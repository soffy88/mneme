...
def register_mocks():
    ...
    class MockLLMCaller:
        async def __call__(self, *, messages: list, max_tokens: int = 1000, **kwargs):
            return {
                "content": "That is wonderful! Can you tell me more about your plans?",
    ...
    ProviderRegistry.register(
        "pronunciation", "aliyun", MockPronunciationCaller(), replace=True
    )
    ProviderRegistry.register(
        "pronunciation", "default", MockPronunciationCaller(), replace=True
    )
        ...
        for turn in turns:
            ...
            for word in discouraging_words:
                assert word not in feedback, (
                    f"Feedback '{feedback}' contains discouraging phrase: '{word}'"
                )
    finally:
...
@pytest.mark.asyncio
async def test_speaking_with_ku_id_updates_mastery(client, student, db):
    """带 ku_id 时，mock 发音分 0.85 ≥ 0.6 阈值 → 判对，KCMastery/InteractionEvent 落库。"""
    app.dependency_overrides[get_current_user] = lambda: student
    try:
        ...
        assert resp.status_code == 200
        assert resp.json()["kc_updated"] is True

        row = (
            await db.execute(
                select(KCMastery).where(
                    KCMastery.student_id == student.id,
                    KCMastery.knowledge_point == KU,
                )
            )
        ).scalar_one_or_none()
        ...
        assert ev.source.value == "speaking"
...
@pytest.mark.asyncio
async def test_speaking_without_ku_id_skips_mastery_update(client, student, db):
    """没有 ku_id（自由主题练习）：不强行归因，KCMastery 不产生任何行。"""
    app.dependency_overrides[get_current_user] = lambda: student
    try:
        resp = await client.post(
            "/v1/speaking/practice",
            json={
                "topic": "Free talk",
                "target_sentences": "Whatever comes to mind.",
                "grade": "Grade 10",
            },
        )
        assert resp.status_code == 200
        assert resp.json()["kc_updated"] is False

        rows = (
            (
                await db.execute(
                    select(KCMastery).where(KCMastery.student_id == student.id)
                )
            )
            .scalars()
            .all()
