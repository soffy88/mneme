...
async def seed_ku(db: AsyncSession):
    ...
    db.add(
        TextbookFile(
            ...
            filename="test.pdf",
            file_type="pdf",
            storage_path="curriculum_standards/test.pdf",
            has_text_layer=True,
        )
    )
    await db.commit()
...
async def test_get_single_ku_with_prereq_mastery(seed_ku, student):
    ...
    async with AsyncClient(
        ...
        r = await c.get(
            f"/v1/knowledge-points/{seed_ku['ku2_id']}",
...
async def test_practice_submit_correct_no_wrong_question_created(
    ...
    bq = (
        await db.execute(select(WrongQuestion).where(WrongQuestion.id == bank_question))
    ).scalar_one_or_none()
    ...
    student_wq = (
        await db.execute(
            select(WrongQuestion).where(
                WrongQuestion.id == uuid.UUID(d["wrong_question_id"])
            )
        )
    ).scalar_one_or_none()
        ...
        (
            await db.execute(
                select(WrongQuestion).where(
                    ...
                    WrongQuestion.question_text.like(f"%{ku2_id[:10]}%"),
                )
            )
        )
        .scalars()
