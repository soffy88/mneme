...
async def db_context():
    ...
    async with session_factory() as session:
        ...
        user = User(
            id=student_id, phone=f"136{str(uuid.uuid4())[:8]}", role=UserRole.student
        )
        session.add(user)
        await session.flush()
...
async def test_paper_analysis_updates_mastery(db_context):
    ...
    with (
        patch("omodul.analyze_paper.ocr_paper", AsyncMock(return_value=mock_ocr_res)),
        patch(
            "omodul.analyze_paper.process_single_question",
            AsyncMock(return_value=mock_grading_res),
        ),
    ):
        ...
        assert findings.cognitive_updates[0]["ku_id"] == "GDMATH-SET-01"
        ...
        # 2. 应存在 kc_mastery 记录
        stmt_mastery = select(KCMastery).where(
            KCMastery.student_id == student_id,
            KCMastery.knowledge_point == "GDMATH-SET-01",
        )
        ...
        stmt_event = select(InteractionEvent).where(
