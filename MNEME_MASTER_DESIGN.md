...
## 附录 · 教育架构重排 P0（2026-07-03 专家评审 v1.0，L0–L2 落地）

从"机制清单"升级为"教育架构"：用教学模型组织机制。P0 三层先行。

### L1 · 统一学习者模型（单一真相源）
所有上层功能对"掌握/到期/难度带/学习阶段/校准"**只读** `services/learner_model.py`，不得再私自定阈值。
四算法唯一职责：IRT=题目标定+能力θ；BKT=KU 掌握 P(mastery)；FSRS=记忆单元保持+调度；
FIRe=前置隐式重复信用（只写调度折扣）。**KU≠记忆单元。**
- **权威阈值（单源）**：`GATE=0.6`（前置放行/薄弱线）、`MASTERED=0.7`（掌握裁决/计数/进巩固）、
  `GREEN=0.75 / YELLOW=0.40`（掌握色）。散落字面量一律迁移引用此处。
- 读 API：`get_mastery / mastery_color / fringe / get_stage / get_zpd_band`。

### L0 · 学习层北极星（飞轮换宪法）
模型层（AUC/校准/FSRS RMSE）与产品层（留存）降为**从属**；一级指标改为学习层四指标：
掌握速度、延迟保持率（保留探针升格）、迁移率、校准度。**红线：留存涨而学习平 = 回滚。** `GET /v1/moat/learning-metrics`。

### L2 · 教学引擎 · 答案分级政策（**苏格拉底红线松绑，据专家评审 + 用户 2026-07-03 批准**）
旧红线"任何模式不输出标准答案/完整步骤"**改为按情境分级**（Bastani 2024：裸答案后测−17%、护栏组无损害
→ 支持答案**分级**非**永不示解**；专长逆转：新手需样例非纯提问）：

| 情境 | 政策 |
|------|------|
| 学生**自带**作业/试卷原题 | **永不**输出可抄答案/完整步骤（旧红线保留于此） |
| **写作/作文** | **永不**代写成段，只标注/提问/给 rubric（红线最严，Fan 2024） |
| 系统教学**同构新知**（worked_example 阶段） | **必须**给完整样例 + 自我解释提示（样例效应） |
| 卡壳升级阶梯 | 元认知→定位→步骤→同构例题；原题始终不给 |

- 学习阶段状态机（每 KU）：`worked_example → completion → retrieval → consolidation`，按 `get_stage` 渐退；
  概念性错回样例层、程序性失误原地重试。
...
## 附录 · L4 语文双轨 + L6 家长端(2026-07-03)

- **L4 语文双轨**：语文非 KU 掌握型。`oprim.chinese_track` 按 ku_type 硬分 **记诵轨**(文言实词虚词/成语/
  字形字音/文化常识/名句名篇→FSRS 记忆单元) / **素养轨**(阅读各类/写作/诗词鉴赏→阅读策略+作文rubric,不套BKT)。
...
## 附录 · L8 红队 CI 门禁(2026-07-03)

答案泄露拦截不能只靠系统提示词，需代码层二次检测兜底（不依赖 LLM 是否被诱导）。契约：

- **essay_guide**：`_looks_like_handoff` 检测代写交接措辞 + 超长(>80字符)无引导问句特征的整段文本，
  命中即替换为引导语并标记 `EssayGuideResult.answer_leaked`（此前该函数零拦截）。
- **socratic_loop**：原样字符串比对 + 空白归一化比对（防"x = 2"绕过"x=2"），仅答案去空白后 ≥2 字符时
  启用归一化，避免单字符答案假阳性。
- **reading_comprehension_guide / physics_force_analysis_guide**：leak 短语表补总结/结论式收尾措辞
  （中英双语种），防角色扮演诱导换个说法给答案、防单语种防线被翻译绕过。
- **已知缺口（不做，非遗漏）**：多轮拆解（答案拆成多轮）无法用无状态单轮/归一化比对防御；
  跨轮累积比对方案因短数字答案假阳性率过高 + 涉及无 git 版本控制的共享平台包（platform/3O，
  hicode 项目共享）改动风险过大，本批次不做。
- 测试：`tests/test_redteam_answer_leak.py`，8 例覆盖角色扮演/"帮我检查答案"/翻译绕过/格式绕过
  四类诱导 × 4 个引导端点，并入默认 pytest 套件（无需额外开关）。
