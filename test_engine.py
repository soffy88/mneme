...
from core.cognitive_state import CognitiveStore, process_interaction, review_queue
from data.guangdong_math_kc import get_bkt_prior


def test_mastery_rises_on_correct():
    s = bkt.new_state_from_prior("GDMATH-CONIC-01", get_bkt_prior("GDMATH-CONIC-01"))
    seq = []
    for _ in range(8):
        bkt.bkt_update(s, True)   # 无遗忘
...
def test_mastery_falls_on_wrong():
    s = bkt.new_state_from_prior("GDMATH-SET-01", get_bkt_prior("GDMATH-SET-01"))
    # 先拉高
    for _ in range(5):
        bkt.bkt_update(s, True)
    high = s.current()
    for _ in range(5):
        bkt.bkt_update(s, False)
...
def test_error_classification():
    ...
    for _ in range(8):
        bkt.bkt_update(s, True)
    assert bkt.classify_error(s) == "careless", "高掌握答错应判粗心"
    print(f"  高掌握(P={s.current():.3f})答错 → {bkt.classify_error(s)}  ✓")

    # 低掌握 + 答错 → 不会
    s2 = bkt.new_state_from_prior("GDMATH-DERIV-03", get_bkt_prior("GDMATH-DERIV-03"))
    assert bkt.classify_error(s2) == "dontknow", "低掌握答错应判不会"
    print(f"  低掌握(P={s2.current():.3f})答错 → {bkt.classify_error(s2)}  ✓")
...
def test_auc():
    ...
    for _ in range(300):
        ...
        s = bkt.new_state_from_prior("GDMATH-SEQ-01", get_bkt_prior("GDMATH-SEQ-01"))
        # 喂 6 道历史题：掌握者多对，未掌握者多错
        for _ in range(6):
            obs = (random.random() < 0.85) if true_mastered else (random.random() < 0.25)
            bkt.bkt_update(s, obs)
        # 预测第 7 题
        p = bkt.predict_correct(s)
