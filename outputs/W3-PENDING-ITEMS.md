...
## 新发现：LLM 兜底出题不带年级上下文（B-8 补测过程中撞见，不修，超出本次范围）
...
学生出了研究生集合论的题。说明兜底出题 prompt 没有把年级/学段传给 LLM，
只给了 KC 名字。这是 W2C 就存在的既有兜底出题逻辑的既有质量问题，Book
Engine 没有引入它、只是通过真实交接测试撞见了它——不在本次修复范围，
留后续（涉及 `services/mcp_router.py` 的 `_llm_generate_question`）。
