...
| B-7 | 三态 UI 阅读器可见 | ✅ |
| B-8 | quiz 块经既有判分护栏，掌握度回流 | ✅ **已补测**（2026-07-19，`book-to-learn-handoff.spec.ts`）：真实点击书页里渲染出来的"去学习"链接（非手写 URL），断言 href 带非空 `?kcs=`；落地后走真实 NextObjective/RequestQuestion/SubmitAnswer UI 事件；断言 `interaction_events` 真的落了一条记录。过程中还修了一个真 bug——链接原来没带 kc_ids，点进去是 `/studio/learn` 默认路径，不是这块讲的知识点 |
...
## ✅ 已修复（2026-07-19）：LLM 兜底出题不带年级上下文

`RENJIAO-G1-MATH-S-ku-1-5的认识`（一年级"1~5的认识"）题库无内容、走
`_llm_generate_question` 兜底出题时，生成的题目是"在集合论与数理逻辑的
皮亚诺公理体系背景下，自然数通常被定义为冯·诺依曼序数……"——面向一年级
学生出了研究生集合论的题。根因：兜底出题 prompt 没有把年级/学段传给
LLM，只给了 KC 名字。这是 W2C 就存在的既有兜底出题逻辑的既有质量问题，
Book Engine 没有引入它，是 B-8 真实交接测试撞见的。

**修复**：`tool_request_question`（`services/mcp_router.py`）取 `kc_name`
的查询改为 `outerjoin(Textbook)` 顺带取 `Textbook.grade`（如 `"G1"`），
传给 `_llm_generate_question(kc_name, grade=...)`；新增 `_GRADE_ZH` 映射
（G1..G12 → 中文学段），prompt 里用真实学段描述替换原来硬编码的"适合
中学生"，并显式要求"不能用更高学段的概念/术语"。

**验证**：`tests/test_llm_generate_question_grade_context.py`（4 测试，含
用真实撞过 bug 的那个 G1 KU 做端到端验证）；对真实 qwen provider 实测，
同一个 G1 KC 现在生成"数一数，下面有几个苹果🍎🍎🍎"（真实符合学段），
G10 KC 生成真实高中难度的二次函数题——不再是研究生集合论。既有
`test_mcp_request_question.py`（3 测试）+ 全量 e2e（含 `expected-leak.
spec.ts` 的人在环连续作答）交叉验证零回归。已部署（`mneme-api-1` 重启
生效）。
