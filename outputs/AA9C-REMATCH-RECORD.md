...
结果：match+verify 挂成 32 道；**全量抽查 32 条后人工撤 1 条存疑**（一道"程序框图算法功能"
题挂到"一元二次不等式求解框图"KC，话题近但存疑）→ **净重挂 31 / 孤儿 162**。其余 130 道无契合
g10-a KC（多为跨课程：立体几何 / 算法框图 / 数论 / 解析几何），**正确留孤儿**。抽查其余 31 条
挂得对（如"映射 f:x→-x²+2x…"→ku004 函数的概念、"-1∈{...}"→元素与集合、Venn 计数题→Venn KC）。
有高一可服务题的 g10-a KC：**30 → 34**。
